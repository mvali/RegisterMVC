...
public class EmailServer
{
    ...
    SmtpClient Smtp()
    {
        SmtpClient sm = new SmtpClient();// not required if it is defined in web.config; or: new SmtpClient("mail.server.com",25);
    ...
    int EmailSendGo(MailType from, MailType to, string body, string subject, List<string> AttachmentFilePath = null)
    {
        try
        {
            ...
            new EmailsSent().Emails_AddNew(mail.To.ToString(), mail.Subject, mail.Body);
            return 0;
        }
        catch (Exception ex)
        {
